...
program
  ...
  .action(async (workflowName, options) => {
    ...
    try {
      ...
      const statusResponse = await sendCommand('status', { name: workflowName });
      ...
      const response = await sendCommand('remove', { name: workflowName, deleteDir: !!options.deleteDir }, (milestone) => {
        switch (milestone.status) {
          case MILESTONE_STATUS.STARTING: {
            spinner.start(milestone.message);
            break;
