...
- **Agent** — A deep module that orchestrates the execution of autonomous coding loops. It internally handles outcome analysis, retry loops, backoffs, and safety checks, delegating specific model execution to an **AgentAdapter**.
- **AgentAdapter** — A seam that encapsulates the specific CLI commands, parameters, and output parsing logic for different underlying models (e.g., Gemini, Pi AI), returning a normalized result to the **Agent**.
